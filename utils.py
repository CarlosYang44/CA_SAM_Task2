...
class FocalDiceloss(nn.Module):
    ...
    def forward(self, pred, mask):
        ...
        loss = focal_loss + self.weight * dice_loss
        return loss
...
class DistillLoss(nn.Module):
    def __init__(self, loss_type="kl", temperature=1.0):
        """
        :param loss_type: str, "kl" for KL divergence or "mse" for MSE loss
        :param temperature: float, temperature for distillation
        :param weight: float, weight for the distillation loss
        """
    ...
    def forward(self, embedding, ori_embedding):
        """
        Forward pass for distillation loss.
        
        :param embedding: torch.Tensor, student model output [B, C, H, W]
        :param ori_embedding: torch.Tensor, teacher model output [B, C, H, W]
        :return: loss value
        """
        # Ensure embeddings have the same shape
        assert embedding.shape == ori_embedding.shape, "Embedding and ori_embedding should have the same shape."
        
        # Pooling to reduce spatial dimensions [B, C, H, W] -> [B, C]
        embedding = self.pooling(embedding).squeeze(-1).squeeze(-1)  # [B, C]
        ori_embedding = self.pooling(ori_embedding).squeeze(-1).squeeze(-1)  # [B, C]

        if self.loss_type == "kl":
            # Normalize logits for numerical stability
            logits_student = embedding / self.temperature
            logits_teacher = ori_embedding / self.temperature
            
            # Compute softmax probabilities
            probs_student = F.log_softmax(logits_student, dim=-1)  # Log-probabilities for student
            probs_teacher = F.softmax(logits_teacher, dim=-1)  # Probabilities for teacher
            
            # KL divergence
            loss = F.kl_div(probs_student, probs_teacher, reduction="batchmean") * (self.temperature ** 2)
        
        elif self.loss_type == "mse":
            # Normalize embeddings
            logits_student = F.normalize(embedding, p=2, dim=-1)
            logits_teacher = F.normalize(ori_embedding, p=2, dim=-1)
            
            # Mean squared error loss
            loss = F.mse_loss(logits_student, logits_teacher)
        
        else:
            raise ValueError(f"Unsupported loss type: {self.loss_type}. Use 'kl' or 'mse'.")
